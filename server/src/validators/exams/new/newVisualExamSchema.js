...
const NewVisualExamSchema = z
	.object({
		color_family: z.string().refine(val => colorFamilyOptions.includes(val), {
			message: `Color family must be ${colorFamilyOptions.join("/")}`,
		}),
		color_shade: z.string().min(1, "Color shade is required"),
		limpidity: z.string().refine(val => limpidityOptions.includes(val), {
			message: `Limpidity must be ${limpidityOptions.join("/")}`,
		}),
		consistency: z.string().refine(val => consistencyOptions.includes(val), {
			message: `Consistency must be ${consistencyOptions.join("/")}`,
		}),
		chains_number: z
			.string()
			.refine(val => chainsNumberOptions.includes(val), {
				message: `Chains number must be ${chainsNumberOptions.join("/")}`,
			})
			.optional(),
		rise_speed: z
			.string()
			.refine(val => riseSpeedOptions.includes(val), {
				message: `Rise speed must be ${riseSpeedOptions.join("/")}`,
			})
			.optional(),
		bubble_size: z
			.string()
			.refine(val => bubblesizeOptions.includes(val), {
				message: `Bubble size must be ${bubblesizeOptions.join("/")}`,
			})
			.optional(),
		bubble_persistence: z
			.string()
			.refine(val => bubblePersistenceOptions.includes(val), {
