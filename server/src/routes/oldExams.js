import { Router } from "express";
import { PrismaClient } from "../generated/prisma/index.js";
import { formatOldOlfactoryExam } from "../utils/tastings.js";
import { OldTasteExamSchema } from "../validators/exams/old/oldTasteExamSchema.js";
import { OldFinalExamSchema } from "../validators/exams/old/oldFinalExamSchema.js";
import { OldVisualExamSchema } from "../validators/exams/old/oldVisualExamSchema.js";
import { OldOlfactoryExamSchema } from "../validators/exams/old/oldOlfactoryExamSchema.js";

const router = Router();
const prisma = new PrismaClient();

const examTypes = {
	visual: {
		model: prisma.visual_exams_old,
		schema: OldVisualExamSchema,
	},
	olfactory: {
		model: prisma.olfactory_exams_old,
		schema: OldOlfactoryExamSchema,
	},
	taste: {
		model: prisma.taste_olfactory_exams_old,
		schema: OldTasteExamSchema,
	},
	final: {
		model: prisma.final_considerations_old,
		schema: OldFinalExamSchema,
	},
};

function omitIdTid(exam) {
	if (!exam) return null;
	delete exam.id;
	delete exam.tid;
	return exam;
}

async function validateTastingOwnership(tid, uid, res) {
	if (!tid || tid.length !== 36) {
		res.status(400).json({ error: `URL parameter 'tasting_uuid' is an invalid UUID-32` });
		return null;
	}

	const tasting = await prisma.tastings.findUnique({ where: { tid, uid } });
	if (!tasting) {
		res.status(404).json({ error: `Tasting ${tid} not found for user ${uid}` });
		return null;
	}

	return tasting;
}

// GET /api/v1/exams
router.get("/", (req, res) => {
	res.status(400).json({ error: `URL parameter 'tasting_uuid' is ${req.params.tid}` });
});

// GET /api/v1/exams/:tid
router.get("/:tid", async (req, res) => {
	const { tid } = req.params;
	const uid = req.user.uid;

	try {
		const tasting = await validateTastingOwnership(tid, uid, res);
		if (!tasting) return;

		const [visualExam, olfactoryExam, tasteOlfactoryExam, finalConsiderations] =
			await Promise.all([
				examTypes["visual"].model.findMany({ where: { tid }, orderBy: { id: "desc" } }),
				examTypes["olfactory"].model.findMany({ where: { tid }, orderBy: { id: "desc" } }),
				examTypes["taste"].model.findMany({ where: { tid }, orderBy: { id: "desc" } }),
				examTypes["final"].model.findMany({ where: { tid }, orderBy: { id: "desc" } }),
			]);

		delete visualExam[0]?.id;
		delete visualExam[0]?.tid;
		delete olfactoryExam[0]?.id;
		delete olfactoryExam[0]?.tid;
		delete tasteOlfactoryExam[0]?.id;
		delete tasteOlfactoryExam[0]?.tid;
		delete finalConsiderations[0]?.id;
		delete finalConsiderations[0]?.tid;

		res.json({
			tasting_uuid: tid,
			exams: {
				visual_exam: visualExam[0] ?? {},
				olfactory_exam: olfactoryExam[0] ? formatOldOlfactoryExam(olfactoryExam[0]) : {},
				taste_olfactory_exam: tasteOlfactoryExam[0] ?? {},
				final_considerations: finalConsiderations[0] ?? {},
			},
		});
	} catch (err) {
		console.error(err);
		res.status(500).json({ error: "Internal Server Error" });
	}
});

// GET /api/v1/exams/:tid/:exam
router.get("/:tid/:exam", async (req, res) => {
	const { tid, exam } = req.params;
	const uid = req.user.uid;

	if (!examTypes[exam]) {
		return res.status(400).json({ error: `Invalid exam type '${exam}'` });
	}

	try {
		const tasting = await validateTastingOwnership(tid, uid, res);
		if (!tasting) return;

		const examData = await examTypes[exam].model.findMany({
			where: { tid },
			orderBy: { id: "desc" },
		});

		delete examData[0]?.id;
		delete examData[0]?.tid;

		let formattedData = examData[0];

		if (exam === "olfactory") {
			formattedData = formatOldOlfactoryExam(formattedData);
		}

		res.json({
			tasting_uuid: tid,
			[`${exam.replace("-", "_")}_exam`]: formattedData ?? {},
		});
	} catch (err) {
		console.error(err);
		res.status(500).json({ error: "Internal Server Error" });
	}
});

// PUT /api/v1/exams/:tid/:exam
router.put("/:tid/:exam", async (req, res) => {
	const { tid, exam } = req.params;
	const uid = req.user.uid;

	if (!examTypes[exam]) {
		return res.status(400).json({ error: `Invalid exam type '${exam}'` });
	}

	const tasting = await validateTastingOwnership(tid, uid, res);
	if (!tasting) return;

	const parsed = examTypes[exam].schema.safeParse(req.body);
	if (!parsed.success) {
		const error = parsed.error.issues[0]?.message;
		return res.status(400).json({ error: error });
	}

	const body = parsed.data;

	try {
		const existingExam = await examTypes[exam].model.findUnique({
			where: { tid },
		});
		if (!existingExam) {
			return res.status(404).json({ error: `${exam} exam not found for tasting ${tid}` });
		}
		const updatedExam = await examTypes[exam].model.update({
			where: { tid },
			data: body,
		});

		let formattedData = updatedExam;
		delete formattedData.id;
		delete formattedData.tid;
		if (exam === "olfactory") {
			formattedData = formatOldOlfactoryExam(updatedExam);
		}

		res.status(200).json(formattedData);
	} catch (err) {
		console.log(err);
		res.status(500).json({ error: `Error updating ${exam} exam` });
	}
});

// POST /api/v1/exams/:tid
router.post("/:tid", async (req, res) => {
	const { tid } = req.params;
	const uid = req.user.uid;

	const tasting = await validateTastingOwnership(tid, uid, res);
	if (!tasting) return;

	const parsedVisual = OldVisualExamSchema.safeParse(req.body.visual_exam);
	const parsedOlfactory = OldOlfactoryExamSchema.safeParse(req.body.olfactory_exam);
	const parsedTaste = OldTasteExamSchema.safeParse(req.body.taste_olfactory_exam);
	const parsedFinal = OldFinalExamSchema.safeParse(req.body.final_considerations);

	if (
		!parsedVisual.success ||
		!parsedOlfactory.success ||
		!parsedTaste.success ||
		!parsedFinal.success
	) {
		const errors = [];
		if (!parsedVisual.success) {
			errors.push(...parsedVisual.error.issues.map(issue => issue.message));
		}
		if (!parsedOlfactory.success) {
			errors.push(...parsedOlfactory.error.issues.map(issue => issue.message));
		}
		if (!parsedTaste.success) {
			errors.push(...parsedTaste.error.issues.map(issue => issue.message));
		}
		if (!parsedFinal.success) {
			errors.push(...parsedFinal.error.issues.map(issue => issue.message));
		}
		return res.status(400).json({ error: errors[0] });
	}

	const visual_exam = parsedVisual.data;
	const olfactory_exam = parsedOlfactory.data;
	const taste_olfactory_exam = parsedTaste.data;
	const final_considerations = parsedFinal.data;

	try {
		const results = await prisma.$transaction([
			visual_exam
				? examTypes["visual"].model.create({
						data: { tastings: { connect: { tid: tid } }, ...visual_exam },
					})
				: Promise.resolve(null),
			olfactory_exam
				? examTypes["olfactory"].model.create({
						data: { tastings: { connect: { tid: tid } }, ...olfactory_exam },
					})
				: Promise.resolve(null),
			taste_olfactory_exam
				? examTypes["taste"].model.create({
						data: { tastings: { connect: { tid: tid } }, ...taste_olfactory_exam },
					})
				: Promise.resolve(null),
			final_considerations
				? examTypes["final"].model.create({
						data: { tastings: { connect: { tid: tid } }, ...final_considerations },
					})
				: Promise.resolve(null),
		]);

		res.status(201).json({
			tasting_uuid: tid,
			exams: {
				visual_exam: omitIdTid(results[0]),
				olfactory_exam: formatOldOlfactoryExam(omitIdTid(results[1])),
				taste_olfactory_exam: omitIdTid(results[2]),
				final_considerations: omitIdTid(results[3]),
			},
		});
	} catch (err) {
		if (err.code === "P2014") {
			return res.status(409).json({ error: `Tasting ${tid} has already been examined` });
		}
		console.error(err);
		res.status(500).json({ error: "Error creating exams" });
	}
});

// POST /api/v1/exams/:tid/:exam
router.post("/:tid/:exam", async (req, res) => {
	const { tid, exam } = req.params;
	const uid = req.user.uid;

	if (!examTypes[exam]) {
		return res.status(400).json({ error: `Invalid exam type '${exam}'` });
	}

	const tasting = await validateTastingOwnership(tid, uid, res);
	if (!tasting) return;

	const parsed = examTypes[exam].schema.safeParse(req.body);
	if (!parsed.success) {
		const error = parsed.error.issues[0]?.message;
		return res.status(400).json({ error: error });
	}

	const body = parsed.data;

	try {
		const newExam = await examTypes[exam].model.create({
			data: {
				tastings: { connect: { tid: tid } },
				...body,
			},
		});

		delete newExam?.id;
		delete newExam?.tid;

		let formattedData = newExam;
		if (exam === "olfactory" && newExam) {
			formattedData = formatOldOlfactoryExam(newExam);
		}
		res.status(201).json(formattedData);
	} catch (err) {
		if (err.code === "P2014") {
			return res.status(409).json({ error: `Tasting ${tid} already has ${exam} exam` });
		}
		console.error(err);
		res.status(500).json({ error: `Error creating ${exam} exam` });
	}
});

export default router;
