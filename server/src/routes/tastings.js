...
router.get('/:tid/exams',
  async (req, res) => {
    ...
    try {
      ...
      const visualExam = await prisma.visual_exams.findMany({
        where: { tid: tid },
        orderBy: {
          id: 'desc'
        }
      ...
      const olfactoryExam = await prisma.olfactory_exams.findMany({
        where: { tid: tid },
        orderBy: {
          id: 'desc'
        }
      ...
      const tasteOlfactoryExam = await prisma.taste_olfactory_exams.findMany({
        where: { tid: tid },
        orderBy: {
          id: 'desc'
        }
      ...
      res.json({ exams: {
        visual: visualExam[0] ?? {},
        olfactory: olfactoryExam[0] ?? {},
        taste_olfactory: tasteOlfactoryExam[0] ?? {}
      }});
    }
    catch (err) {
      console.error(err);
